...
-(void)deleteImage
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(photoWillBeRemoved:)]) {
        [self.delegate photoWillBeRemoved:index];
    }
    ...
    if (_scrollView.subviews.count>1) {
        ...
        [self reSetupScrollView:toIndex];
        _indexLabel.text = [NSString stringWithFormat:@"%d/%ld", toIndex+1, (long)self.imageCount];
...
-(void)reSetupScrollView:(int)curIndex
{
    ...
    if (![self.subviews containsObject:_scrollView]) {
        [self addSubview:_scrollView];
        [self bringToolbarsFront];
    }
    ...
    [self setupImageOfImageViewForIndex:curIndex];
}
...
- (void)layoutSubviews
{
    ...
    if (self.shouldShowDeleteBtn && _delButton) {
        _delButton.frame = CGRectMake(self.bounds.size.width-80, self.bounds.size.height - 70, 50, 25);
    }
}
